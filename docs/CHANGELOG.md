...
## v0.1.1-rc - Telegram ingress + release cleanup
- Added Telegram ingress router that reuses existing core parse/mapping/PO flows (no duplicated business logic).
- Added channel-aware dedup/identity migration (`channel`, `sender_id`, `msg_id`) and `tenant_links`.
- Expanded low-tech onboarding with Windows + WSL + ngrok + Telegram path and explicit checkpoints.
- Performed release clean pass: validators green, docs consistency checks, operational script safety checks.
