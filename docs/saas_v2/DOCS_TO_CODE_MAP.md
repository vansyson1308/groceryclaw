...
| Queue runtime | BullMQ-lite queue (Redis-backed wait + delayed keys) | Producer: `Queue.add`; Consumer: `Worker` loop with retry scheduling | No separate probe endpoint; availability checked via Redis `PING` in service `/readyz` | `REDIS_URL` (+ db/password parsed) | `packages/common/src/bullmq-lite.ts`, `packages/common/src/redis.ts` | `infra/compose/v2/docker-compose.yml` |

## Notes
- Runtime HTTP implementation is Node's built-in `node:http` server, not Fastify.
- Queue implementation is a BullMQ-like Redis shim with persisted attempt metadata and delayed retry scheduling. Service logic still governs domain-level terminal vs retriable classification and DB-side DLQ/audit markers.
