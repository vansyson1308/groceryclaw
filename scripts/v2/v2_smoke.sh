...
health_body="$(curl -sS -w '\n%{http_code}' "http://127.0.0.1:${GATEWAY_HOST_PORT:-8081}/healthz")"
...
ready_body="$(curl -sS -w '\n%{http_code}' "http://127.0.0.1:${GATEWAY_HOST_PORT:-8081}/readyz")"
...
# Flush the wait queue so we start from a known state
...
if [[ "$webhook_code" != "200" ]]; then
  ...
  echo "$webhook_json"
  # Restart worker before exiting
  docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" start worker >/dev/null 2>&1 || true
  exit 1
fi

echo "$webhook_json" | grep -q '"status":"accepted"'

echo "[5/5] Verify queue received at least one job..."
# Small delay to let gateway finish async enqueue
sleep 1
queue_len="$(docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" exec -T redis redis-cli -a "${REDIS_PASSWORD:-redis_dev_password}" --no-auth-warning LLEN "bull-${BULLMQ_QUEUE_NAME:-process-inbound}-wait" | tr -d '\r' | tail -n1)"
if ! [[ "$queue_len" =~ ^[0-9]+$ ]]; then
  echo "Could not read queue length (got: $queue_len)"
  docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" start worker >/dev/null 2>&1 || true
  exit 1
fi
if (( queue_len < 1 )); then
...
# Restart worker to resume normal processing
docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" start worker >/dev/null 2>&1 || true
