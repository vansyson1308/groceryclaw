...
test('parseInvoiceXml rejects adversarially long tag payload quickly', () => {
  const huge = 'a'.repeat(50000);
  const xml = `<invoice><invoiceNumber>${huge}</invoiceNumber><invoiceDate>2026-01-01</invoiceDate><items><item><name>Item A</name><qty>1</qty><unitPrice>10</unitPrice><lineTotal>10</lineTotal></item></items></invoice>`;
  assert.throws(() => parseInvoiceXml(xml), /xml_tag_value_too_large|invalid_invoice_header/);
});
